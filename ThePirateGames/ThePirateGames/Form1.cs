...
namespace ThePirateGames
{
    public partial class Form1 : Form
    {
        ...
        public string sqlTeamScoreString;
        ...
        {
            lobbyJoinID = sqlIDInput.Text;
        }

        private void createPanel_MouseHover(object sender, EventArgs e)
        {
            createPanel.BackColor = System.Drawing.Color.MediumBlue;
        }

        private void createPanel_MouseLeave(object sender, EventArgs e)
        {
            createPanel.BackColor = System.Drawing.Color.Blue;
        }

        private void joinPanel_MouseHover(object sender, EventArgs e)
        {
            joinPanel.BackColor = System.Drawing.Color.Lime;
        }
        ...
        private void createPanel_Click(object sender, EventArgs e)
        {
            ...
            sqlTeamScoreString = "totalScore1";
        }

        private void textBox1_Click(object sender, EventArgs e)
        {
            sqlIDInput.Text = null;
        }
        ...
        {
            sqlPanel.Visible = false;
            lobbyIDPanel.Visible = true;

            sqlTeamScoreString = "totalScore2";
        }

        private void joinPanel_MouseLeave(object sender, EventArgs e)
        {
            joinPanel.BackColor = System.Drawing.Color.LimeGreen;
        }
        ...
        private void lobbyIDStartButton_Click(object sender, EventArgs e)
        {
            string query = string.Format("SELECT isActive FROM pirategames_matches WHERE ID = {0}", lobbyJoinID);
           getActiveState(query);

            if (lobbyJoinID != "" && isActive == 1)
            {
                chooseTeamPanel.Visible = true;
                lobbyIDPanel.Visible = false;
            }
            else if (lobbyJoinID != "" && isActive == 0)
            {
                MessageBox.Show("Lobby is not joinable!");
            }
            else
            {
                MessageBox.Show("Please enter a joinable lobby");
            }
        }

        private void sqlTeamInput_TextChanged(object sender, EventArgs e)
        {
            teamName = sqlTeamInput.Text;
        }

        private void teamNameButton_Click(object sender, EventArgs e)
        {
            teamName = sqlTeamInput.Text;
            gameStart = true;

            if (teamName != "" && lobbyJoinID == "")
            {
               createGame();
                chooseTeamPanel.Visible = false;
                MessageBox.Show(teamName);
                lobbyIDNum.Visible = true;
                lobbyIDNum.Text = ("Lobby ID: " + lobbyID);

            }
            else if (teamName != "" && lobbyID == "")
            {
                joinGame(Convert.ToInt32(totalPoints.Text));
                chooseTeamPanel.Visible = false;
                MessageBox.Show(teamName);

                lobbyIDNum.Visible = true;
                lobbyIDNum.Text = ("Lobby ID: " + lobbyJoinID);
            }
            else
            {
        ...
        private void sqlIDInput_TextChanged(object sender, EventArgs e)
        {
            lobbyJoinID = sqlIDInput.Text;
        }
        ...
        private void closeButton_Click(object sender, EventArgs e)
        {
            if (lobbyID != "")
            {
                string resetQuery = string.Format("UPDATE pirategames_matches SET isActive = 0 WHERE ID = {0};", lobbyID);
                openConnection(resetQuery);
            }
            timer2.Start();
        }
        ...
        private void gameReset(string resetString)
        {
            ...
            switch (dr)
            {
                case DialogResult.Yes:
                    ...
                    //Disabling old game lobbies so cant be joined in the future
                    if (lobbyID != "")
                    {
                        string resetQuery = string.Format("UPDATE pirategames_matches SET isActive = 0 WHERE ID = {0};", lobbyID);
                        openConnection(resetQuery);
        ...
        {
            if (sqlTeamScoreString == "totalScore1")
            {
                string query = string.Format("UPDATE pirategames_matches SET {0} = {1} WHERE ID = {2};", sqlTeamScoreString, totalPoints.Text, lobbyID);
                openConnection(query);
            }
            else if (sqlTeamScoreString == "totalScore2")
            {
                string query = string.Format("UPDATE pirategames_matches SET {0} = {1} WHERE ID = {2};", sqlTeamScoreString, totalPoints.Text, lobbyJoinID);
                openConnection(query);
            }
        ...
        {
            connString = "SERVER= 160.153.48.193;PORT=3306;DATABASE=pirategames;UID=pirateGames;PASSWORD=Ridge07070707!;";
            try
            {
                ...
                string newGameQuery = string.Format("INSERT INTO `pirategames_matches` (`ID`, `Team1` ,`totalScore1`, `isActive`) VALUES (NULL, '{0}' ,'0', '1'); SELECT * FROM pirategames_matches;", teamName);


                MySqlCommand cmd = new MySqlCommand(newGameQuery, conn);
                MySqlDataReader dataReader = cmd.ExecuteReader();

                while (dataReader.Read())
                {
                    lobbyID = dataReader["ID"].ToString();
                }
        ...
        public void joinGame(int totalPoints)
        {
            connString = "SERVER= 160.153.48.193;PORT=3306;DATABASE=pirategames;UID=pirateGames;PASSWORD=Ridge07070707!;";
            ...
            try
            {
                ...
                string joinGameQuery = string.Format("UPDATE pirategames_matches SET Team2 = '{0}', totalScore2 = '{1}' WHERE ID = {2};", teamName, totalPoints, lobbyJoinID);


                MySqlCommand cmd = new MySqlCommand(joinGameQuery, conn);
                MySqlDataReader dataReader = cmd.ExecuteReader();
        ...
        public void getActiveState(string query)
        {
            connString = "SERVER= 160.153.48.193;PORT=3306;DATABASE=pirategames;UID=pirateGames;PASSWORD=Ridge07070707!;";
            try
            {
...
//Finished work on creating a lobby for 1 player. Need to add functionality to joining 
//lobbies by ID and then after that I need to add multi team joinability ~~ Maybe check
//team name columns for a null value???
